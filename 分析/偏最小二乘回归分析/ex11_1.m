...
contr=cumsum(PCTVAR,2) %求累积贡献率  第一行是自变量，第二行是因变量
xw=a\XS  %求自变量提出成分系数，每列对应一个成分，这里xw等于stats.W,例如，第一列对应是u1=xi的系数
yw=b\YS  %求因变量提出成分的系数，第一列对于v1 = yi的系数
ncomp=input('请根据PCTVAR的值确定提出成分对的个数ncomp=');
[XL2,YL2,XS2,YS2,BETA2,PCTVAR2,MSE2,stats2] =plsregress(a,b,ncomp) % XL2第i行对应ui=xj的表达式，YL2每一行对应v，BETA2的第i列对应yi=xi的表达式，0是常数项
...
% repmat(beta3(1,:),[size(a,1),1])：重复常数项
