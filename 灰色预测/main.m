%%  输入原始数据并做出时间序列图
clear;clc
tb1=readtable('data');      % 读表，第一列为时间，第二列为客流数据
Time=tb1(:,1);      % 提取时间
Time=table2cell(Time);      % 时间转换 table→cell
year =datetime(Time,'InputFormat','yyyy年');      % 生成日期时间序列
x0=tb1(:,2);      % 提取客流数据
x0=table2array(x0);      % 客流量格式转换为table→double
...
%%  级比检验                   对一次累加后的数据进行准指数规律的检验(注意，这个检验有时候即使能通过，也不一定能保证预测结果非常好，例如上面的第三组数据)
if ERROR == 0   % 如果上述错误均没有发生时，才能执行下面的操作步骤
    ...
    range = minmax(lamda');
    if(range(1)>exp(1)^(-2/(n+1)) & range(2)<exp(1)^(2/(n+1)))
        disp('级比检验通过')
    else
        disp('级比检验不通过，请考虑平移变换')
    end
...
if ERROR == 0   % 如果上述错误均没有发生时，才能执行下面的操作步骤
    if  n > 4  % 数据量大于4时，将数据分为训练组和试验组(根据原数据量大小n来取，n为5-7个则取最后两年为试验组，n大于7则取最后三年为试验组)
        ...
        train_x0 = x0(1:end-test_num);  % 训练数据  end即为n的数
        disp('训练数据是: ')
