...
flower1 = data1(:,1:4);
T1 = [zeros(50,2),ones(50,1)]; % 001代表1   也可以1，2，3吧
...
P = [P_1',P_2',P_3'];   % 总的测试集（属性）
T = [T_1',T_2',T_3'];   % 总的测试集（类别）
P_test = [P_test1',P_test2',P_test3'];  % 总的测试集（属性）
T_test = [T_test1',T_test2',T_test3'];  % 总的测试集（类别）

net = feedforwardnet(20);
net.trainParam.goal = 0.001;
net.trainParam.epochs = 5000;
net.trainFcn = 'trainrp';  % 训练方法
net.layers{1}.transferFcn = 'tansig';  % 第一层传递函数
net.layers{2}.transferFcn = 'purelin'; % 第二层
net = train(net,P,T);
T_sim = net(P_test) > 0.5;   % 大于0.5就认为是1
